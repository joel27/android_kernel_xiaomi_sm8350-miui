			   ...
			   u32 threshold, u32 prio, u32 prio_max)
{
	 ...
	 */
	entry = zram_entry_alloc(zram, comp_len_new,
			       __GFP_KSWAPD_RECLAIM |
			       __GFP_NOWARN |
			       __GFP_HIGHMEM |
			       __GFP_MOVABLE);
	if (IS_ERR_VALUE(entry)) {
		zcomp_stream_put(zram->comps[prio]);
		return PTR_ERR((void *)entry);
	}

	dst = zs_map_object(zram->mem_pool, entry->handle_new, ZS_MM_WO);
	memcpy(dst, zstrm->buffer, comp_len_new);
	zcomp_stream_put(zram->comps[prio]);

	zs_unmap_object(zram->mem_pool, entry->handle_new);
