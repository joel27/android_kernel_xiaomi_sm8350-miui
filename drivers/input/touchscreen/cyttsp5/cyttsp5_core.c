...
static int cyttsp5_core_sleep_(struct cyttsp5_core_data *cd)
{
	...
	if ((cd->cpdata->flags & CY_CORE_FLAG_POWEROFF_ON_SLEEP) || cd->tpdbg_status)
...
static int cyttsp5_core_wake_device_from_easywake_(struct cyttsp5_core_data *cd)
{
	struct cyttsp5_hid_output hid_output = {
...
static int cyttsp5_core_wake_device_(struct cyttsp5_core_data *cd)
{
	if (!IS_DEEP_SLEEP_CONFIGURED(cd->easy_wakeup_gesture)) {
		...
		wake_up(&cd->wait_q);
		msleep(20);
		return cyttsp5_core_wake_device_from_easywake_(cd);
	}
...
int cyttsp5_core_rt_suspend(struct device *dev)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	int rc;
...
EXPORT_SYMBOL_GPL(cyttsp5_core_rt_suspend);

int cyttsp5_core_rt_resume(struct device *dev)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	int rc;
...
EXPORT_SYMBOL_GPL(cyttsp5_core_rt_resume);
#endif
...
exit:
	cd->pm_suspend = true;
	reinit_completion(&cd->pm_completion);
	return 0;
}
...
static int cyttsp5_hid_output_lockdown_info_(struct cyttsp5_core_data *cd)
{
	struct cyttsp5_lockdown_info *ld_info = &cd->ld_info;
	u16 crc;
	int rc;
	u8 read_buf[12];

	rc = cyttsp5_hid_output_read_conf_block_(cd, 0, 12,
			CY_MDATA_EBID, read_buf, &crc);
	if (rc)
		return rc;

	ld_info->cg_lamination = read_buf[4];
	ld_info->display_maker = read_buf[5];
	ld_info->cg_ink_color = read_buf[6];
	ld_info->hw_ver = read_buf[7];
	ld_info->project_id = get_unaligned_be16(&read_buf[8]);;
	ld_info->cg_maker = read_buf[10];
	ld_info->default_val = read_buf[11];

	return 0;
}


/* Show Lockdown info via sysfs */
static ssize_t cyttsp5_lockdown_info_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	struct cyttsp5_lockdown_info *ldi;
	int rc = 0;
	...
	rc = cyttsp5_hid_output_lockdown_info_(cd);
	...
	if (!rc) {
		...
		return sprintf(buf,
			"Status: %d\n"
			"CG lamination: 0x%02X\n"
			"Display maker: 0x%02X\n"
			"CG Ink color : 0x%02X\n"
			"HW version   : 0x%02X\n"
			"Project ID   : 0x%04X\n"
			"CG maker     : 0x%02X\n"
...
static struct device_attribute attributes[] = {
	...
	__ATTR(lockdown_info, S_IRUGO, cyttsp5_lockdown_info_show, NULL),
};
		...
		unsigned long event, void *data)
{
	...
	if (!cd || !evdata || !evdata->data)
		goto exit;

	if (evdata->disp_id != MI_DISPLAY_SECONDARY) {
		dev_err(cd->dev, "%s: not second display\n", __func__);
		return NOTIFY_OK;
	}
...
static void cyttsp5_setup_fb_notifier(struct cyttsp5_core_data *cd)
{
	...
	rc = mi_disp_register_client(&cd->fb_notifier);
	if (rc)
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	pr_info("%s: start probe core.", __func__);
	...
	init_completion(&cd->pm_completion);
	cd->pm_suspend = false;
...
#elif defined(CONFIG_DRM)
	mi_disp_unregister_client(&cd->fb_notifier);
#endif
