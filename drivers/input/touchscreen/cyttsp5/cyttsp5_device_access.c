...
#ifdef CYTTSP5_DEBUG_FS

static void tpdbg_shutdown(struct cyttsp5_core_data *cd, bool sleep)
{
	if (sleep) {
		cd->tpdbg_status = true;
		cyttsp5_core_rt_suspend(cd->dev);
	} else {
		cyttsp5_core_rt_resume(cd->dev);
	}
}

static void tpdbg_suspend(struct cyttsp5_core_data *cd, bool sleep)
{
	if (sleep)
		cyttsp5_core_rt_suspend(cd->dev);
	else
...
{
	struct cyttsp5_device_access_data *dad = file->private_data;
	struct cyttsp5_core_data *cd;
	struct device *dev;
	...
	if (!strncmp(cmd, "irq-disable", 11))
		...
		tpdbg_shutdown(cd, true);
	else if (!strncmp(cmd, "tp-sd-off", 9))
		tpdbg_shutdown(cd, false);
	else if (!strncmp(cmd, "tp-suspend-en", 13))
		tpdbg_suspend(cd, true);
	else if (!strncmp(cmd, "tp-suspend-off", 14))
		tpdbg_suspend(cd, false);
out:
	kfree(cmd);
...
static int cyttsp5_setup_sysfs(struct device *dev)
{
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("switch_state", 0660,
				dad->base_dentry, dad, &tpdbg_operations))) {
		dev_err(dev, "%s: Error, could not create tpdbg\n");
...
bool cyttsp5_device_access_is_init(void)
{
	return true;
}
EXPORT_SYMBOL_GPL(cyttsp5_device_access_is_init);
