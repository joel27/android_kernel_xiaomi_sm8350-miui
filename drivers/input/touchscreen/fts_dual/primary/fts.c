...
static ssize_t fts_ms_pure_raw_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	int res = 0, count = 0, j = 0, pos =  0, last_pos = 0;
	struct i2c_client *client = to_i2c_client(dev);
	struct fts_ts_info *info = i2c_get_clientdata(client);
	u8 sett = LOAD_SYS_INFO;
	u8 pure_cmd[3] = {FTS_CMD_FRAMEBUFFER_R, 0x00, 0x62};
	u8 pure_data[3];
	int buf_size;
	...
	res = writeSysCmd(SYS_CMD_LOAD_DATA, &sett, 1);
	if (res < OK) {
		logError(1,
	...
	if (fts_writeRead(pure_cmd, 4, pure_data, 3) < OK) {
		logError(1,
			 "%s %s: read error... ERROR %08X \n",
			 tag, __func__, ERROR_BUS_WR);
	}
	offset = (u16)( (pure_data[1]  << 8) |pure_data[0]);
	logError(1, "%s pure_data:%x\n", tag, offset);

	res = fts_disableInterrupt();
	if (res < OK)
		goto END;

	buf_size = force_len * sense_len * 5 + (sense_len + force_len) * 4 + 50;
	...
	data_dump_buf1 = vmalloc(buf_size);
	if (!data_dump_buf1) {
		logError(1, "%s %s alloc all_strbuff fail\n", tag, __func__);
		goto END;
	} else
		memset(data_dump_buf1, 0, buf_size);

	res = fts_writeReadU8UX(FTS_CMD_FRAMEBUFFER_R, BITS_16, offset, (u8 *)data_dump_buf1,
			      node_data_size * BYTES_PER_NODE, DUMMY_FRAMEBUFFER);
	if (res < OK) {
		logError(1, "%s %s getFrameData: ERROR %08X\n", tag, __func__, ERROR_BUS_R);
		return ERROR_BUS_R;
	}	fts_mode_handler(info, 1);
	temp_buf = (short *)data_dump_buf1;
/*
	j = 0;
	for (i = 0; i < node_data_size * BYTES_PER_NODE; i += 2) {
		temp_buf[j] = (short)((data_dump_buf1[i + 1] << 8) + data_dump_buf1[i]);
		j++;
	}
*/
	pos +=
	    snprintf(data_dump_buf + last_pos, buf_size,
		     "MsTouchPureRaw,%2d,%2d\n ,", force_len, sense_len);
	last_pos = pos;
	if (res >= OK) {
		for (j = 0; j < sense_len; j++)
			if ((j + 1) % sense_len) {
				pos +=
				    snprintf(data_dump_buf + last_pos, buf_size,
					     "C%02d,", j);
		...
		for (j = 0; j < sense_len * force_len; j++) {
			if ((j + 1) % sense_len) {
				pos +=
				    snprintf(data_dump_buf + last_pos, buf_size,
					     "%4d,", temp_buf[j]);
	...
	count = strlen(data_dump_buf);
	memcpy(buf, data_dump_buf, count);
	vfree(data_dump_buf);
	vfree(data_dump_buf1);
	data_dump_buf = NULL;
	data_dump_buf1 = NULL;
END:
	fts_enableInterrupt();
...
int fts_enable_touch_raw(bool en)
{
	...
	if (en) {
		fw_name = fts_info->board->htp_fw_name;
		if (fw_name != NULL && !fts_info->enable_htp_fw) {
...
#endif
	pm_stay_awake(info->dev);
	if (info->enable_touch_raw || info->clicktouch_count ||info->ear_sensor) {
...
static void fts_set_grip_rect(int *buf)
{
	...
	x_end = *(buf + 4);
	y_end = *(buf + 5);
	logError(1, "%s grip_type:%d, grip_pos:%d,x_start:%d,y_start:%d,x_end:%d,y_end:%d\n", tag,
	...
	ret = fts_writeRead_dma_safe(grip_rcmd, sizeof(grip_rcmd) / sizeof(u8), grip_value,
			 sizeof(grip_value) / sizeof(u8));
	logError(0, "%s grip_value, type:%d pos:%d, x_start:%d, y_start:%d, x_end:%d, y_end:%d\n",
			tag, type, pos, (grip_value[3] << 8) | grip_value[2], (grip_value[1] << 8) | grip_value[0],
...
static void fts_update_grip_mode(void)
{
	...
	fts_info->orientation_update = false;
}
...
static void fts_update_touchmode_data(void)
{
	...
	if (update) {
		for (j = 2; j < (sizeof(set_cmd) / sizeof(u8) - 1); j++) {
			...
			/*disable always active in gamemode, star-436*/
			else if (j - 2 == Touch_Active_MODE)
				temp_value = 0;
			else
		...
		/* j18 don't use tolerance, > 10 will restore default */
		set_cmd[9] = 11;
		logError(1,
		...
		ret = fts_write_dma_safe(get_cmd, sizeof(get_cmd) / sizeof(u8));
		if (ret < OK) {
			logError(1,
				 "%s %s: error while writing touchmode data ERROR %08X\n",
				 tag, __func__, ret);
...
static void fts_grip_mode_work(struct work_struct *work)
{
	...
	if (fts_info->sensor_sleep) {
		logError(1, "%s %s fts is in sleep\n", tag, __func__);
		return;
	}
	ret = wait_event_interruptible_timeout(fts_info->wait_queue, !(fts_info->irq_status ||
	fts_info->touch_id), msecs_to_jiffies(2000));
	if (ret <= 0) {
		logError(1, "%s %s: wait touch finger up timeout\n", tag, __func__);
	}
	mutex_lock(&fts_info->orientation_update_mutex);
	gamemode_on = xiaomi_touch_interfaces.touch_mode[Touch_Game_Mode][GET_CUR_VALUE];
	...
	if (xiaomi_touch_interfaces.long_mode_len % GRIP_PARAMETER_NUM) {
		...
		mutex_unlock(&fts_info->orientation_update_mutex);
		return;
	}
...
static int fts_set_cur_value(int mode, int value)
{
	...
	if (mode == Touch_Panel_Orientation) {
		fts_info->orientation_update = true;
	}
...
static int fts_set_mode_long_value(int mode, int len, int *buf)
{
	...
	if (mode == Touch_Grip_Mode) {
		...
		if (fts_info->gamemode_enable) {
			...
			/*flush_delayed_work(&fts_info->grip_mode_delay_work);*/
			queue_delayed_work(fts_info->touch_feature_wq, &fts_info->cmd_update_delay_work,
			   msecs_to_jiffies(1));
		}
	}
...
int fts_ear_sensor_write(int value)
{
	int ret = 0;

	fts_info->ear_sensor = !!value;
...
static void fts_resume_work(struct work_struct *work)
{
	...
	wake_up(&info->wait_queue);
	/*flush_delayed_work(&info->grip_mode_delay_work);*/
...
static void fts_suspend_work(struct work_struct *work)
{
	...
	if (info->ear_sensor) {
		logError(1, "%s %s: touch ear sensor, skip\n", tag,
			 __func__);
		return;
	}
...
static void fts_switch_mode_work(struct work_struct *work)
{
	...
	logError(1, "%s %s gesture type:%d, gesture_enable:%d\n", tag, __func__, gesture_type, info->gesture_enabled);
	if (info->resume_bit) {
		logError(1, "%s %s touch in resume mode, don't need to set gesture cmds\n", tag, __func__);
		return;
	} else {
		logError(1, "%s %s touch in suspend mode, will resume and suspend again\n", tag, __func__);
		flush_workqueue(info->event_wq);
		queue_work(info->event_wq, &info->resume_work);
		flush_workqueue(info->event_wq);
		queue_work(info->event_wq, &info->suspend_work);
...
static void *fts_datadump_seq_start(struct seq_file *m, loff_t *pos)
{
	return *pos < 1 ? (void *)1 : NULL;
}

static void *fts_datadump_seq_next(struct seq_file *m, void *v, loff_t *pos)
{
	++*pos;
	return NULL;
}

static void fts_datadump_seq_stop(struct seq_file *m, void *v)
{
	return;
}

static int32_t fts_datadump_seq_show(struct seq_file *m, void *v)
{
	int cnt = 0;
	char *tmp;
	tmp = vmalloc(PAGE_SIZE * 10);
	if (tmp == NULL)
		return 0;
	else
		memset(tmp, 0, PAGE_SIZE * 10);
	cnt = fts_selftest_info_show(fts_info->dev, NULL, tmp);
	cnt += fts_ms_raw_show(fts_info->dev, NULL, tmp + cnt);
	cnt += fts_ms_cx2_lp_show(fts_info->dev, NULL, tmp + cnt);
	cnt += fts_ms_cx2_lp_total_show(fts_info->dev, NULL, tmp + cnt);
	cnt += fts_ss_raw_show(fts_info->dev, NULL, tmp + cnt);
	cnt += fts_ss_ix_total_show(fts_info->dev, NULL, tmp + cnt);
	cnt += fts_mutual_raw_ito_show(fts_info->dev, NULL, tmp + cnt);
	seq_puts(m, tmp);
	vfree(tmp);
	tmp = NULL;
...
static struct seq_operations fts_datadump_seq_ops = {
	.start = fts_datadump_seq_start,
	.next = fts_datadump_seq_next,
	.stop = fts_datadump_seq_stop,
	.show = fts_datadump_seq_show
};
...
static const struct file_operations fts_datadump_ops = {
	.owner = THIS_MODULE,
	.open = fts_datadump_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = seq_release,
};

static int32_t fts_pure_raw_seq_show(struct seq_file *m, void *v)
{
	int cnt = 0;
	char *tmp;
	tmp = vmalloc(PAGE_SIZE * 10);
	if (tmp == NULL)
		...
		memset(tmp, 0, PAGE_SIZE * 10);
	cnt += fts_ms_pure_raw_show(fts_info->dev, NULL, tmp + cnt);
	seq_puts(m, tmp);
	vfree(tmp);
	tmp = NULL;
...
static struct seq_operations fts_pure_raw_seq_ops = {
	.start = fts_datadump_seq_start,
	.next = fts_datadump_seq_next,
	.stop = fts_datadump_seq_stop,
	.show = fts_pure_raw_seq_show
};

static int fts_pure_raw_open(struct inode *inode, struct file *file)
{
...
static int fts_probe(struct spi_device *client)
{
	...
	info->tp_pure_raw_proc =
	    proc_create("tp_pure_raw", 0444, NULL, &fts_pure_raw_ops);
	info->tp_fw_version_proc =
	    proc_create("tp_fw_version", 0444, NULL, &fts_fw_version_ops);

#ifdef FTS_XIAOMI_TOUCHFEATURE
	info->touch_feature_wq =
	    alloc_workqueue("fts-touch-feature-0",
			    WQ_UNBOUND | WQ_HIGHPRI | WQ_CPU_INTENSIVE, 1);
	...
	mutex_init(&info->orientation_update_mutex);
	...
	xiaomi_touch_interfaces.ear_sensor_write = fts_ear_sensor_write;
...
static int fts_remove(struct spi_device *client)
{
	...
	if (info->tp_data_dump_proc)
		remove_proc_entry("tp_data_dump", NULL);
	if (info->tp_pure_raw_proc)
		remove_proc_entry("tp_pure_raw", NULL);
	if (info->tp_fw_version_proc)
		remove_proc_entry("tp_fw_version", NULL);
	info->tp_lockdown_info_proc = NULL;
	info->tp_selftest_proc = NULL;
